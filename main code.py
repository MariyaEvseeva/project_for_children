...
class The_Main_Window(QWidget):
    ...
    def initUI(self):
        ...
        self.image_name.move(10, 5)
        self.image_name.resize(1900, 50)
        self.image_name.setPixmap(self.pixmap_name)
        self.pixmap_name.load("instead of name.jpg")
        self.image_name.setPixmap(self.pixmap_name)

        self.pixmap_angle = QPixmap()
        self.image_angle = QLabel(self)
        self.image_angle.move(1580, 730)
        ...
        if show[0] == 'имя существительное':
            ...
            if p.tag.POS == 'ADJF':
                ...
                if padezh in cases:
                    ...
                    show.append('падеж не определён')
        k = len(show)
        if len([1 for symbol in word if symbol.isalpha() == False]) != 0:
            self.label_morph1.setText('{}'.format('Введи текст без точек, запятых и других символов'))        ##
        elif len([1 for symbol in word if symbol in [chr(i) for i in range(ord('a'), ord('z') + 1)]]) != 0:
            self.label_morph1.setText("Это иностранное слово!")
        else:
            if k == 1:
    ...
    def phonetics(self):
        ...
        if len([1 for symbol in word if symbol.isalpha() == False]) != 0:
            self.label_sounds1.setText('{}'.format('Введи текст без точек, запятых и других символов'))
        elif len([1 for symbol in word if symbol in [chr(i) for i in range(ord('a'), ord('z') + 1)]]) != 0:
            self.label_sounds1.setText("Это иностранное слово!")
        else:
            if n == 1:
